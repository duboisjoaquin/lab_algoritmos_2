...
static bool invrep(abb tree) {
    ...
    if(tree != NULL){
        if (tree->left != NULL){
            inv &= elem_less(tree->left->elem, tree->elem); //los elementos del arbol izquierdo tiene que ser menores al elemento sobre el que estamos parados
        }
        if (tree->right != NULL){
            inv &= elem_less(tree->elem,tree->right->elem); //todos los elementos del arbol derecho tienen que ser mayores al elemento sobre el que estamos parados
        }
        inv &= invrep(tree->left) && invrep(tree->right); //Hacemos llamados a la funcion recursiva para que se fije en los subarboles
    }
    return inv;
}
...
abb abb_add(abb tree, abb_elem e) {
    ...
    if (tree == NULL){ //si el arbol es vacio
        ...
        tree->elem = e; //le asignamos el elemento
        tree->left = NULL;
        tree->right = NULL;
    } else { //si no es vacio
...
unsigned int abb_length(abb tree) {
    ...
    assert(invrep(tree));
    if (tree != NULL){
        length += abb_length(tree->right) + abb_length(tree->left) + 1; //En ese 1 se ve representado el tree.elem en el cual nos encontramos actualmente
    }
